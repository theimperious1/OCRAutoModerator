...
from OCRAutoModerator.config import db_path


async def insert_submission(submission: Submission) -> bool:
    ...
    async with aiosqlite.connect(db_path) as db:
        await db.execute(
            ...
            (submission.id, submission.subreddit.display_name, submission.title, username, submission.url_overridden_by_dest))
        await db.commit()
        return True
